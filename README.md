...
`style` overrides the fields of `Theme` in [`pdformy/context.py`](pdformy/context.py). The most useful ones:
...
Subclass `Block`, declare the fields, implement `render(ctx)` and register the YAML key. The class must be imported (e.g. from [`pdformy/__init__.py`](pdformy/__init__.py)) before any YAML is parsed.
